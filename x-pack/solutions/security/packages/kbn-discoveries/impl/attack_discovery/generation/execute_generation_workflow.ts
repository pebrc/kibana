...
export async function executeGenerationWorkflow({
  ...
  try {
    ...
    const resolveConnector = async () => {
      const { pluginsStart: startPlugins } = await getStartServices();
      const { actions: actionsPlugin } = startPlugins as { actions: ActionsPluginStart };
    ...
    const diagnosticsContext: DiagnosticsContext = {
      config: {
        alertRetrievalMode: workflowConfig.default_alert_retrieval_mode,
    ...
    if (analytics != null) {
      ...
      const usesDefaultRetrieval = workflowConfig.default_alert_retrieval_mode !== 'disabled';
